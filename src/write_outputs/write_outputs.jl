...
function write_outputs(EP::Model, path::AbstractString, setup::Dict, inputs::Dict)
	...
	dfGen = inputs["dfGen"]
	file_name_assumptionsGen = string("gen_assumptions.csv")
	CSV.write(joinpath(path, file_name_assumptionsGen), dfGen)
	...
	if !isempty(inputs["FUSION"])
		write_fusion_summary(path,inputs,setup,EP)
		println("Fusion summary has been written")
		write_fusion(path,inputs,setup,EP)
		println("Fusion time-dependent results have been written")
	end
