...
function write_net_revenue(path::AbstractString, inputs::Dict, setup::Dict, EP::Model, dfCap::DataFrame, dfESRRev::DataFrame, dfResRevenue::DataFrame, dfChargingcost::DataFrame, dfPower::DataFrame, dfEnergyRevenue::DataFrame, dfSubRevenue::DataFrame, dfRegSubRevenue::DataFrame)
	...
	for y in FUSION
		dfNetRevenue.Inv_cost_MW[y] += value.(EP[:eTurbFinalCost][y]) + value.(EP[:eFuel_fixed_costs][y]) + value.(EP[:eVesselFixCosts][y]) + value.(EP[:eFusionTESInvFPPCostOvershoot][y])
	end
	for y in FUSION_ThermStor
		# dfNetRevenue.Inv_cost_MW[y] += value.(EP[:eTESTurbineCost][y])
		dfNetRevenue.Inv_cost_MWh[y] += value.(EP[:eTESCost][y])
	end
	...
	for y in FUSION
		dfNetRevenue.Var_OM_cost_out[y] += value.(EP[:eVesselVarCosts][y]) + value.(EP[:eFusionTESFixedFPPCostOvershoot][y])
	end
	if setup["ParameterScale"] == 1
	...
	for y in FUSION
		for t in 1:T
			dfNetRevenue.Fuel_cost[y] += value.(EP[:deuterium_var_costs][y, t])
		end
	end
	if setup["ParameterScale"] == 1
	...
	# Add a cost for the capacity constraint
	capacity_constraint_dual = zeros(G)
	for y in 1:G
		if dfGen[y, :Max_Cap_MW] > 0
			capacity_constraint_dual[y] += -dual.(EP[:cMaxCap][y])
			println("Max Cap dual for generator ", y, ": ", -dual.(EP[:cMaxCap][y]))
		end
		if dfGen[y, :Min_Cap_MW] > 0
			capacity_constraint_dual[y] += dual.(EP[:cMinCap][y])
			println("Min Cap dual for generator ", y, ": ", dual.(EP[:cMinCap][y]))
		end
		if dfGen[y, :Max_Cap_MWh] > 0
			capacity_constraint_dual[y] += -dual.(EP[:cMaxCapEnergy][y])
			println("Max CapE dual for generator ", y, ": ", -dual.(EP[:cMaxCapEnergy][y]))
		end
		if dfGen[y, :Min_Cap_MWh] > 0
			capacity_constraint_dual[y] += dual.(EP[:cMinCapEnergy][y])
			println("Min CapE dual for generator ", y, ": ", dual.(EP[:cMinCapEnergy][y]))
		end
	end
	dfNetRevenue.CapacityConstraintCost = capacity_constraint_dual
