...
function configure_gurobi(solver_settings_path::String)
	...
	MyNodeFileStart = 0.5 	# Memory threshold for writing node files to disk.
		if(haskey(solver_settings, "NodeFileStart")) MyNodeFileStart = solver_settings["NodeFileStart"] end
	########################################################################
