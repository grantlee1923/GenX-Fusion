...
function non_served_energy!(EP::Model, inputs::Dict, setup::Dict)
	...
	@variable(EP, vServed_Load[t=1:T,z=1:Z] >= 0)
	@constraint(EP, cServedLoad[t=1:T,z=1:Z], vServed_Load[t,z] == inputs["pD"][t,z] - sum(EP[:vNSE][s,t,z] for s=1:SEG))
	@expression(EP, eGross_load[t=1:T,z=1:Z], vServed_Load[t,z] + sum(EP[:vNSE][s,t,z] for s=1:SEG))
end
